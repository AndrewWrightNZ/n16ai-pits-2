...
class OGC3DTile extends THREE.Object3D {
  ...
  constructor(properties) {
    ...
    if (!!properties.domWidth && !!properties.domHeight) {
      this.rendererSize = new THREE.Vector2(
        properties.domWidth,
        properties.domHeight
      );
    } else {
      this.rendererSize = new THREE.Vector2(1000, 1000);
    }
    this.loadingStrategy = properties.loadingStrategy
      ? properties.loadingStrategy.toUpperCase()
      : "INCREMENTAL";
    this.distanceBias = Math.max(
      0.0001,
      properties.distanceBias ? properties.distanceBias : 1
    );
    this.proxy = properties.proxy;
    this.drawBoundingVolume = properties.drawBoundingVolume
      ? properties.drawBoundingVolume
      : false;
    ...
    if (this.occlusionCullingService) {
      ...
      this.colorID =
        (THREE.MathUtils.clamp(self.color.r * 255, 0, 255) << 16) ^
        (THREE.MathUtils.clamp(self.color.g * 255, 0, 255) << 8) ^
        (THREE.MathUtils.clamp(self.color.b * 255, 0, 255) << 0);
    }
    if (this.static) {
      ...
      fetchFunction()
        .then((result) => {
          ...
          result
            .json()
            .then((json) => {
              return resolveImplicite(json, url);
            })
            .then((json) => {
              self._setup({
  ...
  async _setup(properties) {
    ...
    if (properties.json.extensionsRequired) {
      if (
        properties.json.extensionsRequired.includes(
          "JDULTRA_gaussian_splats"
        ) ||
        properties.json.extensionsRequired.includes(
          "JDULTRA_gaussian_splats_V2"
        )
      ) {
    ...
    if (!!properties.json.root) {
      ...
      if (!self.json.geometricError)
        self.json.geometricError = properties.json.geometricError;
      if (!self.json.transform) self.json.transform = properties.json.transform;
      if (!self.json.boundingVolume)
        self.json.boundingVolume = properties.json.boundingVolume;
    } else {
    ...
    self.rootPath = !!properties.json.rootPath
      ? properties.json.rootPath
      : properties.rootPath;
    ...
    if (!!self.json.boundingVolume) {
      if (!!self.json.boundingVolume.box) {
        ...
        self._transformWGS84ToCartesian(
          region[0],
          region[1],
          region[4],
          tempVec1
        ...
        self.boundingVolume = new THREE.Sphere(
          new THREE.Vector3(tempVec1.x, tempVec1.y, tempVec1.z),
          tempVec1.distanceTo(tempVec2)
        );
    ...
    if (!!self.json.content) {
      ...
      if (self.hasMeshContent == 0) {
        self.level = Math.max(
          0,
          self.parentTile ? self.parentTile.level + 0.01 : 0.0
        );
      }
      self._load();
    ...
    if (!!self.centerModel) {
      ...
      if (!!this.json.boundingVolume.region) {
        this._transformWGS84ToCartesian(
          (this.json.boundingVolume.region[0] +
            this.json.boundingVolume.region[2]) *
            0.5,
          (this.json.boundingVolume.region[1] +
            this.json.boundingVolume.region[3]) *
            0.5,
          (this.json.boundingVolume.region[4] +
            this.json.boundingVolume.region[5]) *
            0.5,
          tempVec1
        );
    ...
    if (self.level > 0 && self.drawBoundingVolume) {
      ...
      if (this.boundingVolume.aabb) {
        ...
        self.bbox = new THREE.Box3Helper(
          box,
  ...
  _assembleURL(root, relative) {
    ...
    for (let i = 1; i <= rootParts.length; i++) {
      ...
      const rootToken = rootParts
        .slice(rootParts.length - i, rootParts.length)
        .join("/");
    ...
    return `${rootUrl.protocol}//${rootUrl.host}/${[
      ...rootParts,
      ...relativeParts,
    ].join("/")}`;
  }
  ...
  async _load(loadJson = true, loadMesh = true) {
    ...
    if (!!self.json.content) {
      ...
      let promises = self.json.contents.map((content, index) =>
        loadContent(content, index, loadJson, loadMesh)
      );
              ...
              !self.cameraOnLoad
                ...
                : () => {
                    ...
                    if (self.loadingStrategy == "INCREMENTAL") {
                      if (self.parentTile) {
                        return (
                          self.parentTile._calculateDistanceToCamera(
                            self.cameraOnLoad
                          ) / Math.max(1, self.parentTile.level)
                        );
                      } else {
                        return (
                          self._calculateDistanceToCamera(self.cameraOnLoad) /
                          Math.max(1, self.level)
                        );
                      }
                      /* if (self.parentTile) {
  ...
  dispose() {
    ...
    if (!!self.abortController && !self.jsonRequested) {
      // abort tile request
  ...
  update(camera) {
    ...
    if (this.refine == "REPLACE") {
      switch (this.loadingStrategy) {
        case "IMMEDIATE":
          this._updateImmediate(camera, frustum);
          this._statsImmediate(
            maxLOD,
            numTiles,
            percentageLoaded,
            numTilesRendered
    ...
    return {
      numTilesLoaded: numTiles[0],
      numTilesRendered: numTilesRendered[0],
      maxLOD: maxLOD[0],
      percentageLoaded: percentageLoaded[0],
    };
  }
      ...
      if (self.materialVisibility && (!self.splatsMesh || !!self.splatsReady)) {
        if (parentDisplaysMesh) {
          ...
          if (allChildrenReady && self.childrenTiles.length > 0) {
            ...
            self.childrenTiles.forEach((child) => {
              child._updateNodeVisibilityImmediate(
                !self.splatsMesh || !!self.splatsReady
              );
            });
          }
        }
  ...
  _shouldBeVisibleUpdateImmediate() {
    ...
    if (!self.hasMeshContent) {
      ...
      self.metric <
      self.geometricErrorMultiplier * self.geometricError
    ) {
  ...
  _loadMeshImmediate() {
    ...
    if (self.shouldBeVisible) {
      if (
        self.meshContent.length < self.hasMeshContent &&
  ...
  _computeMetricRecursive(camera, frustum) {
    ...
    self.childrenTiles.forEach((child) =>
      child._computeMetricRecursive(camera, frustum)
    );
  }
    ...
    if (!self.hasUnloadedJSONContent) {
      if (!self.hasMeshContent) {
        if (
          !!self.json &&
          !!self.json.children &&
          self.childrenTiles.length < self.json.children.length
        ) {
        ...
        if (
          ...
          if (
            self.metric >= 0 &&
            self.metric < self.geometricErrorMultiplier * self.geometricError &&
            !!self.json &&
            !!self.json.children &&
            self.childrenTiles.length < self.json.children.length
          ) {
  ...
  _update(camera, frustum) {
    ...
    function _updateTree(metric) {
      ...
      if (
        !self.hasMeshContent ||
        (metric <= self.geometricErrorMultiplier * self.geometricError &&
          (self.meshContent.length > 0 || self.splatsMesh))
      ) {
        if (
          !!self.json &&
          !!self.json.children &&
  ...
  _loadJsonChildren(camera) {
    ...
    for (let i = self.json.children.length - 1; i >= 0; i--) {
      if (
        !self.json.children[i].root &&
        !self.json.children[i].children &&
        !self.json.children[i].getChildren &&
        !self.json.children[i].content &&
        !self.json.children[i].contents
      ) {
  ...
  _isReadyImmediate() {
    if (
      !!this.materialVisibility ||
      (!this.loadOutsideView && this.metric < 0)
    ) {
      return true;
    } else {
      /* if (this.splatsMesh) {
  ...
  _calculateUpdateMetric(camera, frustum) {
    ...
    if (this.boundingVolume instanceof OBB) {
      ...
      distance = Math.max(
        0,
        tempOBB.distanceToPoint(camera.position) - camera.near
      );
      ...
      distance = Math.max(
        0,
        camera.position.distanceTo(tempSphere.center) -
          tempSphere.radius -
          camera.near
      );
    } else {
    ...
    let lambda =
      2.0 * Math.tan(0.5 * fov * 0.01745329251994329576923690768489) * distance;
  ...
  _calculateDistanceToCamera(camera) {
    if (this.boundingVolume instanceof OBB) {
      ...
      return Math.max(
        0,
        camera.position.distanceTo(tempSphere.center) - tempSphere.radius
      );
    } else {
      console.error("unsupported shape");
      return -1;
  ...
  setGeometricErrorMultiplier(geometricErrorMultiplier) {
    ...
    this.childrenTiles.forEach((child) =>
      child.setGeometricErrorMultiplier(geometricErrorMultiplier)
    );
  }
